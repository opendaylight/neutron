...
public class NeutronTrunkJAXBTest {
    ...
    public void test_NeutronTrunk_JAXB() throws JAXBException {
        ...
        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing Status failed", "DOWN",
        ...
        Assert.assertTrue("NeutronTrunk JAXB Test 5 : Testing AdminStateUp failed",
                neutronObject.getAdminStateUp());

        Assert.assertEquals("NeutronTrunk JAXB Test 2: Testing tenant_id failed",
                "cc3641789c8a4304abaa841c64f638d9", neutronObject.getTenantID());

        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing PotId failed", "60aac28d-1d3a-48d9-99bc-dd4bd62e50f2",
                neutronObject.getPortId());

        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing SubPorts size failed", 2,
                neutronObject.getSubPorts().size());

        NeutronTrunkSubPort subPort = neutronObject.getSubPorts().get(0);

        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing SubPort Segmentation Type failed", "vlan",
                subPort.getSegmentationType());

        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing SubPort Segmentation Id failed", "101",
                subPort.getSegmentationId());

        Assert.assertEquals("NeutronTrunk JAXB Test 4 : Testing SubPort Port Id failed",
