...
import org.opendaylight.yang.gen.v1.urn.opendaylight.neutron.l2gateways.rev150712.l2gatewayconnections.attributes.L2gatewayConnections;
import org.opendaylight.yang.gen.v1.urn.opendaylight.neutron.l2gateways.rev150712.l2gatewayconnections.attributes.l2gatewayconnections.L2gatewayConnection;
import org.opendaylight.yang.gen.v1.urn.opendaylight.neutron.l2gateways.rev150712.l2gatewayconnections.attributes.l2gatewayconnections.L2gatewayConnectionBuilder;
...
implements INeutronL2gatewayConnectionCRUD {
    ...
    @Override
    public List<NeutronL2gatewayConnection> getAllNeutronL2gatewayConnections() {
        return getAll();
    }
    ...
    @Override
    public boolean removeNeutronL2gatewayConnection(String uuid) {
        return remove(uuid);
    }

    @Override
    public boolean neutronL2gatewayConnectionInUse(String l2gatewayConnectionID) {
    ...
    protected NeutronL2gatewayConnection fromMd(L2gatewayConnection l2gatewayConnection){
        ...
        if (l2gatewayConnection.getUuid().getValue() != null){
            result.setID(String.valueOf(l2gatewayConnection.getUuid() .getValue()));
        }
        if (l2gatewayConnection.getKey().getUuid().getValue() != null){
            result.setID(String.valueOf(l2gatewayConnection.getKey().getUuid().getValue()));
        }
