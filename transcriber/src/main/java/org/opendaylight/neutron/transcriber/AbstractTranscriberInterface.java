...
public abstract class AbstractTranscriberInterface<
    ...
    protected void addMd(S neutronObject) throws TransactionCommitFailedException {
    ...
    protected boolean updateMd(S neutronObject) throws TransactionCommitFailedException {
    ...
    protected void removeMd(T item) throws TransactionCommitFailedException {
    ...
    private boolean exists(String uuid, ReadTransaction tx) throws ReadFailedException {
        Preconditions.checkNotNull(tx);
    ...
    @Override
    public boolean exists(String uuid) throws ReadFailedException {
        try (ReadOnlyTransaction tx = getDataBroker().newReadOnlyTransaction()) {
    ...
    private boolean update(String uuid, S delta, ReadWriteTransaction tx) throws OperationFailedException {
        Preconditions.checkNotNull(tx);
        if (!exists(uuid, tx)) {
            tx.cancel();
            return false;
    ...
    @Override
    public boolean update(String uuid, S delta) throws OperationFailedException {
        int retries = RETRY_MAX;
        while (retries-- >= 0) {
            final ReadWriteTransaction tx = getDataBroker().newReadWriteTransaction();
            try {
