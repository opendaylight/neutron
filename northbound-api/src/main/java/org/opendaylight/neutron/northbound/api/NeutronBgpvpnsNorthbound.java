...
public class NeutronBgpvpnsNorthbound {
    ...
    private static final String INTERFACE_NAME = "Bgpvpn CRUD Interface";
    private static final String UUID_NO_EXIST = "Bgpvpn UUID does not exist.";
    ...
    public Response showBgpvpn(
        ...
        INeutronBgpvpnCRUD bgpvpnInterface = getNeutronInterfaces().getBgpvpnInterface();
        if (!bgpvpnInterface.bgpvpnExists(netUUID)) {
            throw new ResourceNotFoundException(UUID_NO_EXIST);
        }
        if (fields.size() > 0) {
            NeutronBgpvpn ans = bgpvpnInterface.getBgpvpn(netUUID);
            return Response.status(HttpURLConnection.HTTP_OK).entity(
                    new NeutronBgpvpnRequest(extractFields(ans, fields))).build();
        } else {
            return Response.status(HttpURLConnection.HTTP_OK).entity(
                    new NeutronBgpvpnRequest(bgpvpnInterface.getBgpvpn(netUUID))).build();
        }
    }
    ...
    public Response updateBgpvpn(
            @PathParam("netUUID") String netUUID, final NeutronBgpvpnRequest input
            ) {
        INeutronBgpvpnCRUD bgpvpnInterface = getNeutronInterfaces().getBgpvpnInterface();

        NeutronBgpvpn updatedObject = input.getSingleton();
        NeutronBgpvpn original = bgpvpnInterface.getBgpvpn(netUUID);
        ...
        bgpvpnInterface.updateBgpvpn(netUUID, updatedObject);
    ...
    public Response deleteBgpvpn(
        ...
        NeutronBgpvpn singleton = bgpvpnInterface.getBgpvpn(netUUID);

        if (!bgpvpnInterface.removeBgpvpn(netUUID)) {
