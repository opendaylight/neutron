...
public class NeutronQosPolicyNorthbound extends
    ...
    public Response listGroups(
        ...
        @QueryParam("bandwidth_limit_rules") List queryBandwidthLimitRule,
        @QueryParam("dscp_marking_rules") List queryDscpMarkingRule,
        // pagination
        ...
        List<NeutronQosPolicy> ans = new ArrayList<NeutronQosPolicy>();
        for (NeutronQosPolicy nsg : qosPolicyInterface.getAll()) {
            if ((queryQosPolicyUUID == null || queryQosPolicyUUID.equals(nsg.getID())) &&
                ...
                &&
                (queryQosPolicyIsShared == null || queryQosPolicyIsShared.
                    equals(nsg.getQosPolicyName()))
                &&
                (queryBandwidthLimitRule == null || queryBandwidthLimitRule.
                    equals(nsg.getBwLimitRules()))
                &&
                (queryDscpMarkingRule == null || queryDscpMarkingRule.equals(nsg.getDscpRules()))) {
                if (fields.size() > 0) {
    ...
    @StatusCodes({
        ...
        @ResponseCode(code = HttpURLConnection.HTTP_NOT_IMPLEMENTED, condition = "Not Implemented"),
        @ResponseCode(code = HttpURLConnection.
            HTTP_UNAVAILABLE, condition = "No providers available") })
