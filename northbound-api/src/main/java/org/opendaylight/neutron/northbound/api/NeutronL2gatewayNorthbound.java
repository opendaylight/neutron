...
public class NeutronL2gatewayNorthbound {
    ...
    private static final String UUID_NO_EXIST = "L2gateway does not exist.";
    ...
    public Response createL2gateway(final NeutronL2gatewayRequest input) {
        ...
        if (input.isSingleton()) {
            ...
            if (l2gatewayInterface.neutronL2gatewayExists(singleton.getID())) {
    ...
    public Response listL2gateways(
                                   ...
                                   @QueryParam("devices") String queryNeutronL2gwDevice,
                                   // pagination
        ...
        List<NeutronL2gateway> allL2gateways =
                l2gatewayInterface.getAllNeutronL2gateways();
        ...
        while (i.hasNext()) {
            ...
            if ((queryID == null || queryID.equals(l2gateway.getID()))
                    ...
                    && (queryNeutronL2gwDevice == null || queryNeutronL2gwDevice
                    .equals(l2gateway.getNeutronL2gwDevices()))) {
                if (fields.size() > 0) {
    ...
    public Response showL2gateway(@PathParam ("l2gatewayID") String l2gatewayID,
        ...
        if (!l2gatewayInterface.neutronL2gatewayExists(l2gatewayID)) {
            throw new ResourceNotFoundException(UUID_NO_EXIST);
        }
        if (!fields.isEmpty()) {
            NeutronL2gateway ans = l2gatewayInterface.getNeutronL2gateway(l2gatewayID);
            return Response.status(HttpURLConnection.HTTP_OK)
                    .entity(new NeutronL2gatewayRequest(extractFields(ans, fields))).build();
        }
            return Response.status(HttpURLConnection.HTTP_OK).entity(new NeutronL2gatewayRequest(
                      l2gatewayInterface.getNeutronL2gateway(l2gatewayID))).build();
    }
    ...
    public Response deleteL2gateway(
        ...
        INeutronL2gatewayCRUD l2gatewayInterface = getNeutronInterfaces().getL2gatewayInterface();
        NeutronL2gateway singleton = l2gatewayInterface.getNeutronL2gateway(l2gatewayID);
        if (!l2gatewayInterface.removeNeutronL2gateway(l2gatewayID)){
    ...
    public Response updateL2gateway(
        ...
        INeutronL2gatewayCRUD l2gatewayInterface = getNeutronInterfaces().getL2gatewayInterface();
        NeutronL2gateway original = l2gatewayInterface.getNeutronL2gateway(l2gatewayID);

        NeutronL2gateway updatedObject = input.getSingleton();
        updatedObject.setID(l2gatewayID);
        updatedObject.setTenantID(original.getTenantID());

        // update l2gateway and return the modified object
        l2gatewayInterface.updateNeutronL2gateway(l2gatewayID, updatedObject);
